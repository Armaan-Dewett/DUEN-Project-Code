...
class BoothApp(tk.Tk):
    ...
    def _run_panorama(self):
        try:
            ...
            self._ui(lambda: self._apply_phase("capturing", "Preparing camera..."))
            setup_image_folders()
            warmup_camera()
            if self._check_stop():
                return
            ...
            # Blurry-first-photo fix:
            # 1. move to first angle
            # 2. settle (servo + IMU)
            # 3. discard capture flushes AGC
            # 4. small pause, then begin the real loop
            first_angle = ANGLES_TO_CAPTURE[0]
            self._ui(lambda fa=first_angle: self.message_label.config(
                text=f"Settling at {fa} deg before first shot..."))
            move_servo(first_angle)
            t_end = time.time() + FIRST_SHOT_SETTLE_S
            ...
            for idx, angle in enumerate(ANGLES_TO_CAPTURE):
                if self._check_stop():
                    return
                if idx != 0:
                    move_servo(angle)
                    time.sleep(FAST_SERVO_WAIT)
                self._ui(lambda i=idx, a=angle: self._update_capture_ui(i, a))
                self.lights.flash_once(brightness_fraction=1.0, duration_s=0.08)
                captured_path = take_picture(angle, idx)
                self._ui(lambda p=captured_path: self._show_captured_frame(p))
            ...
            self._ui(lambda: self._apply_phase("stitching", "Loading images..."))
            ok = stitch_images(progress_cb=self._stitch_progress_cb)
            if not ok:
                self._ui(lambda: self._apply_phase(
                    "error",
                    "Stitch failed. Try better lighting or steadier subject."
                ))
                return
            if self._check_stop():
                return
