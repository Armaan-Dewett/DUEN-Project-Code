...
# ── AUTO FLASH / DARK SCENE DETECTION ────────────────────────
# This does not turn the light on by itself. The UI calls scene_is_dark(),
# then controls LightController before capture if the scene is dark.
AUTO_FLASH_ENABLED        = True
AUTO_FLASH_DARK_THRESHOLD = 70    # 0 = black, 255 = white
AUTO_FLASH_BRIGHTNESS     = 220   # ring brightness when dark
AUTO_FLASH_SETTLE_S       = 0.45  # let exposure adjust after ring turns on
...
def estimate_scene_brightness():
  """
  Estimate scene brightness from the live low-resolution camera frame.

  Returns a brightness value from 0 to 255.
  Lower means darker.
  """
  if HEADLESS or cv2 is None or np is None:
      return 255.0
  ...
  try:
      ...
      print(f"[auto flash] brightness check failed: {e}")
      return 255.0


def scene_is_dark(threshold=AUTO_FLASH_DARK_THRESHOLD):
  """Return (is_dark, brightness)."""
  brightness = estimate_scene_brightness()
  is_dark = brightness < threshold
  print(f"[auto flash] brightness={brightness:.1f}, threshold={threshold}, dark={is_dark}")
  return is_dark, brightness
